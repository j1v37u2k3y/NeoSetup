...
class TestOperatorValidation(unittest.TestCase):
    ...
    def test_array_item_enum_invalid_tag(self):
        """An operator_tag outside the allowed set is flagged on that element"""
        test_operator = self.base_operator()
        test_operator["operator_tags"] = ["development", "not_a_real_tag"]

        results = self.validate(test_operator)
        # The valid tag at index 0 is fine; only index 1 is flagged.
        self.assertEqual(len(errors_for(results, "operator_tags[0]")), 0)
        tag_errors = errors_for(results, "operator_tags[1]")
        self.assertEqual(len(tag_errors), 1, "invalid tag should be an item-enum error")
        self.assertIn("not in allowed values", tag_errors[0].message)

    def test_array_item_enum_valid_tags(self):
        """All-valid operator_tags produce no error"""
        test_operator = self.base_operator()
        test_operator["operator_tags"] = ["development", "security", "productivity"]
    ...
    def test_array_item_pattern_enforced(self):
        ...
        test_operator = self.base_operator()
        test_operator["shell_config"]["paths"] = ["$HOME/bin", "/bad path/with spaces"]

        results = self.validate(test_operator)
        self.assertEqual(len(errors_for(results, "shell_config.paths[0]")), 0)
        path_errors = errors_for(results, "shell_config.paths[1]")
        self.assertEqual(len(path_errors), 1, "path with spaces should be an item-pattern error")
